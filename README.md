...
```
hooks/todos/useReadTodos.ts              # GET
hooks/todos/useCreateTodo.ts             # POST
hooks/todos/useUpdateTodo.ts             # PUT
hooks/todos/useUpdateTodoComplete.ts     # PATCH（完了トグル）
hooks/todos/useDeleteTodo.ts             # DELETE
hooks/todos/useCreateSubtask.ts          # POST（サブタスク）
hooks/todos/useDeleteSubtask.ts          # DELETE（サブタスク）
hooks/todos/useUpdateSubtaskComplete.ts  # PATCH（サブタスク完了トグル）
hooks/todos/useTodosCache.ts             # キャッシュ操作ユーティリティ
hooks/tags/useReadTags.ts                # GET（タグ）
hooks/tags/useCreateTag.ts               # POST（タグ）
hooks/tags/useDeleteTag.ts               # DELETE（タグ）
```
