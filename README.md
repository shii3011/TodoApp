...
[![CI](https://img.shields.io/github/actions/workflow/status/YOUR_GITHUB_USERNAME/TodoApp/ci.yml?label=CI&logo=githubactions&logoColor=white)](https://github.com/YOUR_GITHUB_USERNAME/TodoApp/actions)
...
| 対策 | 実装方法 |
|------|---------|
| **シークレット管理** | 本番の機密情報（DB URL・Cognito ID）は Secrets Manager で管理。Lambda 環境変数には ARN のみ |
| **認証** | `aws-jwt-verify` で Cognito JWT を署名・有効期限・発行元まで検証 |
| **レートリミット** | IP ベース（200 req/15 分）+ 認証済みユーザー ID ベース（100 req/15 分）の二重防護 |
...
| **タイムアウト** | 10 秒超過で 503 を返しリソース占有を防止 |
| **非 root コンテナ** | バックエンド・フロントエンドともに `USER node` で実行 |
| **CORS** | `ALLOWED_ORIGINS` 環境変数で許可オリジンを明示管理 |
| **GitHub Actions** | OIDC 認証で長期アクセスキーを排除。main ブランチのみ Assume 可、セッション 30 分 |
...
| **API Gateway** | HTTP API・Lambda プロキシ統合 | バックエンドの HTTP エンドポイント |
| **Lambda** | Node.js 22・メモリ 512MB | Express アプリの実行環境 |
| **Cognito** | パスワード 12 文字以上・メール復旧のみ | ユーザー認証・JWT 発行 |
| **Neon** | `sslmode=require` | サーバーレス PostgreSQL |
| **Secrets Manager** | シークレット名: `todo-app/production` | 機密情報の安全な管理 |

### デプロイ

```bash
# 前提: aws configure 設定済み・jq インストール済み・Docker 起動中
export DATABASE_URL="postgresql://..."
./scripts/deploy.sh
```

CDK でリソースを構築 → フロントエンドをビルド → S3 にアップロード → CloudFront キャッシュ削除
